...
namespace BackendFirmaKolejowaTesty.db.repository
{
    class CompanyDatabaseTests
    {
        ...
        public void Setup()
        {
            ...
            connection.Open();
            var options = new DbContextOptionsBuilder<TrainContext>().UseSqlite(connection).Options;
            var trainContext = new TrainContext(options);
            trainContext.Database.EnsureCreated();
            _database = new CompanyDatabase(trainContext);
        }
        ...
        public void TrainAdding()
        {
            ...
            var first = trains.ToList().First();
            Console.WriteLine(first.id);
            Console.WriteLine(first.capacity);
            Console.WriteLine(first.is_active);
